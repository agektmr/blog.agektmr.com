...
## パスキーで本当にセキュリティは向上するのか？

FIDO は公開鍵暗号方式を使った所有認証が基本です。認証器に物理的にアクセスできない限り、認証を突破するのは難しいというのがポイントでした。ところが、パスキーはこれを同期可能にし、複数端末で利用可能にしました。ここには利点と欠点が存在します。

欠点としては、パスキーが端末の外に出ることになった以上、リスクが増してしまうという点です。検証してみましょう。
