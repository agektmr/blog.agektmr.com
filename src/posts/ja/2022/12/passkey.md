...
**2024/12/26 更新：** パスキーが登場して時間が経ち、パスキー単体で二要素認証かどうかについては意見が変わってきました。FIDO2 認証（パスキー以前の WebAuthn を使った認証）の場合、クレデンシャルをデバイスのセキュアなチップに保存し、ローカル認証も確実にそのデバイスが持つ機能を使うため、二要素認証に間違いありませんでした。しかし、パスキーになってクレデンシャルの保存先はユーザーが自由に選択したパスワードマネージャーであり、ローカル認証もパスワードマネージャーの指定するものになったため、ローカル認証の証明がどこまで信頼できるか分かりません。1Password のようにローカル認証をスキップしても UV フラグを `true` で返すパスワードマネージャーもあります。

これらを踏まえ、僕はパスキーは二要素認証ではない、という結論に達しました。ただ、かといって二要素認証ではないから弱い、とも思いません。むしろ、パスキーのお陰で二要素認証かどうかで強度を測る時代は終わったのだと理解しています。この辺りについて詳しくは、[小岩井さんの記事](https://sizu.me/kkoiwai/posts/vrbdbodcbsub)や [Yuriy Ackermann の記事](https://herrjemand.medium.com/are-passkeys-mfa-f2720c983d5e)を読むことをおすすめします。
